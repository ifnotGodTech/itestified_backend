...
- **In progress**: Phase 15 (Testimony Reactions — Slices 1-2 completed 2026-07-29, Slice 3 (re-icon Favorite) not started).
...
- mobile: pills render real counts and the current user's active reaction; tapping switches/removes correctly; guest tap shows a sign-in prompt instead of a silent no-op; Favorite/bookmark still works exactly as before, just with its new icon — all implemented and passing as of Slice 2, see its note above

Status: Slices 1-2 completed 2026-07-29 — see each slice's notes above. Slice 3 (re-icon Favorite) not started.
