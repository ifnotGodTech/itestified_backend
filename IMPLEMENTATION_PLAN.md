...
Status: in progress — Slices 1-7 are now solid and (for the mobile slices) live-verified in production. See the post-implementation review notes below for what changed in this pass. Not ready to mark Completed until the cross-cutting admin-auth hardening item (`authentication_classes` on the admin donation views) is addressed — this is the one remaining open item for the whole phase.
