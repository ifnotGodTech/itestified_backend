...
Status: In progress -- drafted 2026-07-31 following the immersive-home concept review (Artifact: "iTestified — Immersive Home Concept"), all open decisions resolved in that same discussion. Slice 1 shipped 2026-07-31 (see its note above); Slices 2-6 remain. Slice ordering front-loads the two independent, mobile-only wins (carousel, streak pill) before the larger feed-architecture change (Slice 3), then layers video autoplay and the promo-card system on top of that new foundation.
