...
Status: Completed. All 3 slices implemented 2026-07-28 — see each slice's notes above. Full suites re-run clean: backend 265 tests (same pre-existing 21 `apps.authn` failures/errors, unrelated), `manage.py check` and `makemigrations --check` clean; dashboard 175 tests, `tsc --noEmit` clean; mobile's new test files (`social_links_controller_test.dart`, `follow_screen_test.dart`, `ratings_screen_test.dart`) and `flutter analyze` on the changed files pass — a full `test/features/profile/` directory re-run was still in progress as this entry was written.
