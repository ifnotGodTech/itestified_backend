...
- **Slice 5 — Wire the mock screens to the real endpoints** — replace both "Coming soon" messages with the real request/response flow now that Slices 3-4 both exist
  - Not started. For Delete Account, the mobile screen's local reason strings (the exact display labels shown in the radio list) will need to be mapped to `AccountDeletionReason`'s stable wire codes when this is wired up, rather than sent as-is.
...
Status: Slices 1-4 completed 2026-07-29 — see each slice's notes above. Slice 5 (wiring both mock screens to their now-existing real endpoints) not started.
