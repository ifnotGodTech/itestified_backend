...
- backend (`apps.profile_content`, 13 tests): admin permission matrix, unknown-key rejection, upsert create/overwrite, public endpoint always returns every key (blank rather than missing for an unset one), email/phone format accepted/rejected per key, and a regression guard that key-specific validation doesn't leak onto About Us/Terms/Privacy
- dashboard (4 tests): renders every key with empty defaults, pre-fills a configured key's body, renders success/error states, support email renders as a real `type="email"` input rather than a text area

Status: Slices 1-5 completed 2026-07-28 — see each slice's notes above. Full suites re-run clean: backend 278 tests (same pre-existing 21 `apps.authn` failures/errors, unrelated), `manage.py check` and `makemigrations --check` clean; dashboard 179 tests, `tsc --noEmit` clean; mobile's new test files (`social_links_controller_test.dart`, `follow_screen_test.dart`, `ratings_screen_test.dart`) and `flutter analyze` on the changed files pass, and a subsequent full `test/features/profile/` directory re-run (22 passing) confirmed the only 2 failures are the pre-existing live-network-dependent `profile_flow_test.dart` cases already documented elsewhere in this plan, not a regression. Slices 6-7 (the Help FAQ list becoming dashboard-editable, plus wiring mobile to consume Slices 4-5's content) still planned, not yet built — see each slice's notes above for the intended design.
