...
- **Slice 4 — Admin edits About Us / Terms of Use / Privacy Policy from the dashboard** — admin opens a settings screen with one text area per document, edits the copy, and saves; mobile then shows the updated text without an app release
  - Planned, not started. New `apps.profile_content` app: `ProfileContentBlock` (`key` unique per a fixed `ProfileContentKey` enum — `about_us`, `terms_of_use`, `privacy_policy` — plus `body` text, `updated_by`), mirroring the `apps.social_links`/`apps.app_versions` fixed-key-set pattern rather than an arbitrary admin-creatable-document model. Seeded via a data migration with the *current* hardcoded copy from `about_policy_screen.dart` so nothing goes blank on rollout. Admin `GET`/`PUT` per key (`IsAuthenticated, IsActiveAdmin`); dashboard settings screen with three text areas, same save-flow shape as "Follow links."
- **Slice 5 — Admin sets the support email and phone number from the dashboard** — admin edits the two fields shown in Help's "Contact our support" section; today these are real values but still hardcoded in the Flutter source (fixed there after the Phase 7 contact-info bug), so correcting them again would require another store release
  - Planned, not started. Reuses Slice 4's `ProfileContentBlock` model rather than a new table for two fields: two more fixed keys, `support_email` and `support_phone`. Likely lands on the same dashboard screen as Slice 4 (email/phone rendered as single-line inputs rather than text areas), since both are simple key→value fields for the same audit family.
- **Slice 6 — Admin manages the Help FAQ list from the dashboard** — admin adds, edits, reorders, and removes FAQ question/answer pairs; only active ones show in the app
  - Planned, not started. New `HelpFaqEntry` model (`question`, `answer`, `display_order`, `is_active`, `updated_by`) — a dynamic list, not a fixed key set, so this gets full CRUD rather than Slice 4/5's per-key upsert, closer in shape to the existing inspirational-picture-category admin management than to "Follow links." Seeded via a data migration with the 4 existing hardcoded FAQ items from `help_screen.dart` so nothing goes blank on rollout. Own dashboard screen (list + add/edit/remove/reorder), since entries aren't a fixed set like Slices 4-5.
- **Slice 7 — Mobile renders About/Terms/Privacy, Help FAQ, and support contact from the backend** — `AboutPolicyScreen` and `HelpScreen` (both the FAQ list and the contact section) fetch this content instead of using the hardcoded strings/list currently baked into the Dart source
  - Planned, not started. Same fetch-and-render shape as Slice 2's `FollowScreen`.
...
Status: Slices 1-3 completed 2026-07-28 — see each slice's notes above. Full suites re-run clean at that point: backend 265 tests (same pre-existing 21 `apps.authn` failures/errors, unrelated), `manage.py check` and `makemigrations --check` clean; dashboard 175 tests, `tsc --noEmit` clean; mobile's new test files (`social_links_controller_test.dart`, `follow_screen_test.dart`, `ratings_screen_test.dart`) and `flutter analyze` on the changed files pass, and a subsequent full `test/features/profile/` directory re-run (22 passing) confirmed the only 2 failures are the pre-existing live-network-dependent `profile_flow_test.dart` cases already documented elsewhere in this plan, not a regression. Slices 4-7 (About/Terms/Privacy, support contact info, and the Help FAQ list all becoming dashboard-editable) planned 2026-07-28 per the admin's request, not yet built — see each slice's notes above for the intended design.
