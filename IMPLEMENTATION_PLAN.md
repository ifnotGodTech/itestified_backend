...
- **In progress**: Phase 17 (Scripture Streak — Slices 1-3 completed 2026-07-30, Slice 4 not started; the Slice 3 reminder push ships with a known open gap around per-user timezone, sending at one fixed UTC time instead).
...
Status: Slices 1-3 completed 2026-07-30 — see their notes above. Slice 4 not started. The timezone gap for the reminder push (Slice 3) shipped as a known, documented limitation (one fixed UTC send time) rather than a blocker.
