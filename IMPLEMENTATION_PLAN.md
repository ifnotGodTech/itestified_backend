...
- backend (`apps.profile_content`, 8 tests): admin permission matrix, unknown-key rejection, upsert create/overwrite, public endpoint always returns all three keys (blank rather than missing for an unset one)
- dashboard (3 tests): renders every key with empty defaults, pre-fills a configured key's body, renders success/error states

Status: Slices 1-4 completed 2026-07-28 — see each slice's notes above. Full suites re-run clean: backend 273 tests (same pre-existing 21 `apps.authn` failures/errors, unrelated), `manage.py check` and `makemigrations --check` clean; dashboard 178 tests, `tsc --noEmit` clean; mobile's new test files (`social_links_controller_test.dart`, `follow_screen_test.dart`, `ratings_screen_test.dart`) and `flutter analyze` on the changed files pass, and a subsequent full `test/features/profile/` directory re-run (22 passing) confirmed the only 2 failures are the pre-existing live-network-dependent `profile_flow_test.dart` cases already documented elsewhere in this plan, not a regression. Slices 5-7 (support contact info and the Help FAQ list becoming dashboard-editable, plus wiring mobile to consume Slice 4's content) still planned, not yet built — see each slice's notes above for the intended design.
