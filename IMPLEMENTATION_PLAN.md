...
- **Slice 2 — Delete Account screen (mock)** — user opens "Delete Account" from Profile and sees real consequences copy plus a real confirmation step (not a single accidental tap away); confirming shows a "Coming soon" message rather than doing nothing or pretending to succeed
  - Implemented 2026-07-29. New `DeleteAccountScreen`, replacing the generic placeholder. Real consequences copy (profile, testimonies, comments, favorites, giving history) plus an explicit "I understand..." checkbox that gates the destructive button (disabled until checked) rather than a single always-enabled tap. Confirming shows an "Account deletion is coming soon." SnackBar. 2 new widget tests cover the button's disabled-until-confirmed state and the success path.
- **Slice 3 — Backend: change password while authenticated** — user submits their current password and a new one; the backend verifies the current password is correct before accepting the change, and rejects a new password that fails Django's configured password validators
  - Not started.
- **Slice 4 — Backend: delete my own account** — user confirms account deletion (current password re-entry required, similar in spirit to the change-password slice, so this can't be triggered by someone who grabbed an unlocked phone); the backend revokes the session/auth token immediately and applies whatever data-handling decision Slice 4 above settles on
  - Not started — blocked on the soft-delete/hard-delete and existing-content product decision.
- **Slice 5 — Wire the mock screens to the real endpoints** — replace both "Coming soon" messages with the real request/response flow once Slices 3-4 exist
  - Not started.

Test:
- mobile: both mock screens render with real validation (Change Password rejects a mismatched confirmation and a too-short password; Delete Account requires the explicit confirmation step before the destructive action is reachable) and show the "Coming soon" feedback rather than silently doing nothing
- backend (once Slices 3-4 are built): current-password verification (accepts correct, rejects incorrect), new password validator enforcement, deletion actually revokes the session, and whatever the data-retention decision requires

Status: Slices 1-2 (mobile mock UI) completed 2026-07-29 — see each slice's notes above. `flutter analyze` (whole project) clean; 6 new tests pass. Slices 3-5 (real backend + wiring) not started, pending the account-deletion data-retention decision (soft-delete vs hard-delete, and what happens to a deleted user's existing testimonies/donations/comments).
