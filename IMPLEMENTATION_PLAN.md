...
- **Slice 4 — Backend: delete my own account** — user confirms account deletion (current password re-entry required, similar in spirit to the change-password slice, so this can't be triggered by someone who grabbed an unlocked phone) and submits their reason for leaving, captured for retention/product analysis; the backend revokes the session/auth token immediately and applies whatever data-handling decision Slice 4 above settles on
  - Not started — blocked on the soft-delete/hard-delete and existing-content product decision. Also needs a place to actually store the deletion reason (e.g. a small `AccountDeletionFeedback` record, written before the user row itself is removed/anonymized) — a real decision for this slice, not yet made.
...
Test:
- mobile: both mock screens render with real validation (Change Password rejects a mismatched confirmation and a too-short password; Delete Account requires a reason, requires detail text when "Other" is picked, and requires the explicit confirmation checkbox before the destructive action is reachable) and show the "Coming soon" feedback rather than silently doing nothing
- backend (once Slices 3-4 are built): current-password verification (accepts correct, rejects incorrect), new password validator enforcement, deletion actually revokes the session, the deletion reason is actually persisted somewhere, and whatever the data-retention decision requires

Status: Slices 1-3 completed 2026-07-29 — see each slice's notes above. Slice 4 (delete-my-account backend) not started, blocked on the account-deletion data-retention decision (soft-delete vs hard-delete, what happens to a deleted user's existing testimonies/donations/comments, and where the deletion reason gets stored). Slice 5 (wiring both mock screens to their real endpoints) not started — Slice 3's endpoint already exists for Change Password, just not yet called from the mobile screen.
